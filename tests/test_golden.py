...
KNOWN_DEVIATIONS: dict[str, tuple[str, ...]] = {
    "农用": (
        # ── 用户素材自身矛盾，非系统缺陷（Task 5：资产状况逐因素描述）──
        # 实勘表「离城中心距离」手写为「距离区政府直线距离约300KM」，金样报告写的是
        # 「…约3KM」。资产状况三张表的描述格按 Task 4/5 的设计逐字复现实勘表 D 列
        # （手写依据只进报告、不进算术，不做任何改写），系统忠实输出 Excel 的值
        # （300KM），故与金样对不上。多半是估价师誊写时多打了两个 0，修复须由用户
        # 订正素材，代码无从判断谁对——300 公里显然不是「离城中心距离」的合理量级。
        "距离区政府直线距离约3KM",
    ),
    "办公": (
        # ── 用户素材自身矛盾，非系统缺陷 ──
        ...
        # ── 资产状况逐因素描述：素材句末标点习惯与金样表格不同，非系统缺陷 ──
        # （Task 5）资产状况三张表按实勘表 D 列逐因素描述原样进报告，不做任何改写。
        # 实勘表里这些描述估价师手写成完整句、句末带句号；金样报告表格当年是人工
        # 誊写的短语式描述，未必逐句加句号——办公金样这十三处恰好都省略了句末
        # 句号。系统忠实复现 Excel 原文（含句末句号），与金样对不上的只有这一个
        # 标点字符，取的是归一化后整段做集合比较，逐条列出以下十三处：纯标点
        # 差异，事实无误。
        "705路 ; 732B路 ; 732路 ; 753路 ; 761路 ; 763路 ; 772路 ; 7736路公交车",
        "估价对象平均内净高2.7m-2.8m",
        "估价对象所在宗地东至市心路，南至金惠路，西至无名路，北至博学路",
        "周边1公里范围内有5幢以上办公场所",
        "地面：地毯；墙面：乳胶漆、玻璃隔断；顶面：集成吊顶、石膏板吊顶",
        "外墙是玻璃幕墙，造型比较方正，简洁利落，门面是无框玻璃门",
        "套内无卫生间，设施完善，且具有新风机组、防火排烟、中央空调、消防监控等设施设备，配有垂直客梯10部、货梯2部",
        "建成于2019年，目前未发现不均匀沉降，地面、墙面、门窗保养维护较好，设备设施也维护较好",
        "所在层数为第十二层和第十五层",
        "该宗地红线内外基础设施达到“五通一平”即通给水、通排水、通电、通路、通讯、场地平整",
        "距离北干街道直线距离约1.5公里，萧山区政府直线距离约0.3公里",
        "距离地铁口直线距离约800米",
        "钢混结构，地上23层，地下3层",
        "附近学校有杭州市萧山区第一中等职业学校、杭州市萧山第五高级中学；医院有社区卫生院、萧山医院；银行有中国工商银行、萧山农商银行；商场有萧山万象汇、加州阳光",
        # ── 用户素材自身矛盾，非系统缺陷（Task 5）──
        # 实勘表「智能化程度」手写为「…电器、空调、照面等自动化控制」，金样报告
        # 写的是「…照明等自动化控制」。「照面」显然是「照明」的形近字手误，系统
        # 忠实输出 Excel 原文，故与金样对不上，修复须由用户订正素材。
        "有基本的楼宇门禁、监控系统，电器、空调、照明等自动化控制",
    ),
    "商业": (
        # ── 参数化带来的措辞差异，事实无误 ──
        ...
        # ── 用户素材自身矛盾/标点习惯不同，非系统缺陷（Task 5：资产状况逐因素描述）──
        # 实勘表「公共服务设施」手写「银行有中国邮政银行、杭州银行」，金样报告写的是
        # 「…中国邮政储蓄银行…」（全称）。系统忠实输出 Excel 原文（缺「储蓄」二字），
        # 故与金样对不上，修复须由用户订正素材。
        # 「平均层高」范围号 Excel 用半角连字符「-」，金样报告用全角破折号「—」；
        # 「宽深比」比号 Excel 用全角冒号「：」，金样报告用半角冒号「:」（比例记法
        # 惯例半角，如「16:9」）。两处都是估价师 Excel 里的标点习惯与历史人工报告
        # 排版习惯不同，系统忠实输出 Excel 原文标点，非系统缺陷。
        "附近学校有杭州市钱塘区崇德实验学校、杭州市钱塘区义蓬中学；医院有杭州市第九人民医院，义蓬社区卫生服务站；银行有中国邮政储蓄银行、杭州银行；商场有义蓬购物中心。",
        "估价对象平均层高3.5m—4.0m。",
        "估价对象宽深比为1:2.5，形状规则。",
    ),
}
