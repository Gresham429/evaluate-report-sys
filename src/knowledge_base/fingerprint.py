...
**不覆盖 `Factor.row`**：行号是 Excel 的排版细节，落在 ADR-001 划的知识边界之外
（那份表列的是因素/档次/系数/分值标尺，没有行号）。版本号该跟着知识走，不跟着
排版走。

> 注意：别拿「基础表插一个空行 → 行号后移 → 不该出新版本」当理由——这个场景
> **根本到不了指纹**。`extract_knowledge` 把因素块钉死在 3..30 行、分值标尺钉死
> 在第 31 行，插一行就让分值行移到 32，导入直接抛 `ValueError`（实测）。排除
> row 的理由是「它不是知识」，不是那个不存在的场景。

（row 仍要**存**，Excel 路径靠它定位比较法表的行，见 `adapter.py`；「存」与
「进指纹」是两回事。）
