...
Expected: 输出 `三类共有段落（归一化后）：N 段`，**N 应为 63**。

**判据说明**：`extract_copy.py` 只统计 `len(p) > 12` 的段落（滤掉「序号」「权利人」这类表头短段）。该口径下三份金样的实测基线为：
...
归一化稳定多归并 20 段——正是原本因 `1.` vs `1、`、`见诸` vs `见诸于` 等漂移而被判成三份各异的法定套话。若 N ≤ 43，说明 `drift.py` 规则未生效，返回 Task 6 检查。
