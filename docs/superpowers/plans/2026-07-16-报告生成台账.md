...
> 注：`src/ledger/__init__.py` 引用了 `src.ledger.store`，Task 4 才建。本任务先只写
> `model.py` 与一个空的 `__init__.py`（只导出 model 里的东西），Task 4 再补 store 的导出。
...
把 `src/ledger/__init__.py` 补成 Task 3 里给出的完整版本（含 store 的导出）。
