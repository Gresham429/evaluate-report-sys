...
def test_frozen_exe_can_read_it() -> None:
    ...
    **查 AST 而非扫文本**：文本扫描分不清「代码 import 了它」与「docstring 在解释
    为什么别 import 它」，会逼得文档不能直呼其名——那是测试在惩罚文档写得清楚。
    两种写法都要拦：`import importlib.metadata` 与 `from importlib import metadata`，
    只拦一种等于没拦。
    """
    tree = ast.parse((_ROOT / "src" / "version.py").read_text(encoding="utf-8"))
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            assert all(not a.name.startswith("importlib.metadata") for a in node.names)
        elif isinstance(node, ast.ImportFrom):
            assert not (node.module == "importlib"
                        and any(a.name == "metadata" for a in node.names))
            assert node.module != "importlib.metadata"
```

该文件顶部 import 区需要 `import ast`。
...
- Produces:
  ...
  - `src.ledger.model.MethodUse(名称: str, 版本: str)`
  - `src.ledger.model.InstanceUse(实例: Instance, 编号: str, 判断依据: str)`
  - `src.ledger.model.LedgerEntry(记录号, 报告编号, 生成时间, 经手人, 程序版本, 类别, 基础表: BaseTableUse | None, 估价对象档次: dict[str,str] | None, 实例: tuple[InstanceUse, ...] | None, 方法: MethodUse | None, 权重: tuple[float,...] | None, 结果: Result | None, 一览表: tuple[dict, ...])`
...
def replay(entry: LedgerEntry) -> Result:
    ...
    """
    if (
        entry.基础表 is None or entry.估价对象档次 is None or entry.实例 is None
        or entry.方法 is None or entry.权重 is None
    ):
        # 这六者同生共灭（写入路径 new() 已强制）。任一为 None，即这份报告未经系统
        # 重算、数值取自 Excel，没有可重放的计算过程。
        raise ValueError(
    ...
    result = compute(
        source, [use.实例 for use in entry.实例], entry.权重,
...
- [x] Task 6 生成报告时落台账
- [x] Task 7 台账界面 + 导入警告
- [x] Task 7.5 台账按报告编号分组（用户中途新增，纯展示层）
- [x] Task 8 文档与收尾
```
