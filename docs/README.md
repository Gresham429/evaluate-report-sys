...
- **进行中：钉钉全公司同步**（承载层=**钉钉多维表**，宜搭因专业版弃用）——三个 Notable 后端 + 客户端 + 切换 + 领号已实现，
  **525 项全绿 + 三后端全部真机验通**（三表已建）；仍待接 app 启动 + 登录鉴权 + docx 归档，见 §1b。
...
**仍待**（各自后续小步，均非承载层核心）：**接 app 启动 + 离线待同步/联网对账**（设计已成文、决策已定，见
[离线待同步 design](superpowers/specs/2026-07-18-离线待同步与联网对账-design.md)——app 默认联网、报告编号改多维表统一发号、
离线出报告存「待同步」草稿、联网逐条同步或删；实现须真机点前端）；**钉钉登录鉴权 + 按人裁剪缓存**（§7b，须先设计）；
