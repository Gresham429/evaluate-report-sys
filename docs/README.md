...
- **进行中：钉钉全公司同步**（承载层=**钉钉多维表**，宜搭因专业版弃用）——三个 Notable 后端 + 客户端 + 切换 + 领号已并 main；
  **离线待同步 + 联网对账 + 统一发号**已在 `feat/offline-sync` 实现（**544 项全绿**、mypy/ruff 干净，**前端待真机验收后并 main**）；
  仍待钉钉登录鉴权 + docx 归档，见 §1b。
...
**已实现（`feat/offline-sync`，544 绿、待真机验收 + 并 main）**：**接 app 启动 + 离线待同步/联网对账 + 统一发号**（见
...
整套改动 gate 在 `承载后端=多维表`，**本地模式（默认／交付 exe 无 .env）行为一字不变**。⚠️ 前端单文件改动须真机点一遍（§5 坑 2/4）后再并 main。
