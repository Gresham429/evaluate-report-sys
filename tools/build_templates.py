...
SUBSTITUTIONS: dict[str, list[tuple[str, str]]] = {
    "农用": [
        ...
        ("农用地（耕地）", "{{ usage }}"),
        # ⚠️ 顺序要紧：长串先替换。「2026年4月27日」同时出现在出具日期、
        # 作业期区间、有效期区间三处，先做无差别替换会把后两句切断。
        ("2026年4月20日至2026年4月27日。", "{{ work_period }}。"),
        (
            "（2026年4月27日至2027年4月26日止）",
            "（{{ issue_date_cn }}至{{ validity_end_cn }}止）",
        ),
        ("2026年4月27日", "{{ issue_date_cn }}"),
        ("土地使用权面积50.00亩", "{{ scale }}"),
        # 「出租面积为50.00亩。」是独立成段的项目数据，金样写死。
        # 农用类只有面积一项，直接用合计面积。
        ("出租面积为50.00亩。", "出租面积为{{ total_area }}亩。"),
        (
    ...
    "办公": [
        ...
        # 顺序要紧，同农用注释。
        ("2026年3月26日至2026年6月5日。", "{{ work_period }}。"),
        (
            "（2026年6月5日至2027年6月4日止）",
            "（{{ issue_date_cn }}至{{ validity_end_cn }}止）",
        ),
        ("2026年6月5日", "{{ issue_date_cn }}"),
    ...
    "商业": [
        ...
        # 顺序要紧，同农用注释。
        ("2026年3月25日至2026年4月1日。", "{{ work_period }}。"),
        (
            "（2026年4月1日至2027年3月31日止）",
            "（{{ issue_date_cn }}至{{ validity_end_cn }}止）",
        ),
        ("2026年4月1日", "{{ issue_date_cn }}"),
        ("用途为商业", "用途为{{ usage }}"),
        # 「依据不足假设」段里嵌着逐个对象的面积枚举，是项目数据。
        # 商业类此句措辞与办公不同（办公用「估价对象范围为…」），故单列。
        # Excel 的 C14 规模字段正是这段描述，直接接 {{ scale }}。
        (
            "委托人确认房屋建筑面积为130平方米（其中义蓬中路477号房屋建筑面积60平方米、"
            "487号房屋建筑面积70平方米）",
            "委托人确认{{ scale }}",
        ),
        # 「建筑规模」表格单元格里的逐对象枚举，与办公段214 同类。
        # 复用 {{ subjects_narrative }}（办公亦用此键），三类口径一致。
        (
            "出租面积为 130 平方米。其中义蓬中路477号房屋建筑面积60平方米、"
            "487号房屋建筑面积70平方米。",
            "{{ subjects_narrative }}。",
        ),
        # 「估价范围」句里的规模描述，与 C14 规模字段同义，接 {{ scale }}。
        (
            "总建筑面积130平方米（义蓬中路477号房屋建筑面积60平方米、"
            "487号房屋建筑面积70平方米）及其分摊的土地使用权",
            "{{ scale }}",
        ),
        (
