...
def build() -> None:
    ...
    h1(doc, "一、两种用法")
    bullets(doc, [
        "单机离线：双击程序在本机用，数据全程留在本机、不联外网。适合一人一机独立出报告。",
        "联钉钉全公司：估价师用钉钉小程序现场实勘、办公端登录后拉取问卷出报告，"
        "报告编号、实例库、审核在公司范围内统一。是否联钉钉由部署时的配置决定。",
    ])
    note(doc, "本说明前半部分（出报告、实例库、基础表、台账）两种用法都适用；"
              "「钉钉全公司协作」一节仅在联钉钉时用得到。")
    ...
    h1(doc, "三、第一次用：让本机有基础表")
    p(doc, "基础表是估价知识本身——比较因素、每个因素的档次描述、每差一档的修正系数。"
           "没有它，表单给不出档次选项，也算不出结果。它恒存在本机，供出报告时读取；"
           "拿到本机有两条路，按你的用法二选一：")
    p(doc, "联钉钉（公司统一维护）：七类知识已存在公司钉钉多维表。到「基础表」页点"
           "「从多维表拉取」，一次把各类知识拉到本机即可出报告——不必自己导 Excel。"
           "知识由管理员在源头维护、更新后重新同步，各人再拉一次即取到新版（旧版不覆盖）。")
    p(doc, "单机离线（本机自管）：到「基础表」页，选类别，导入对应的 Excel"
           "（如「办公实勘表、比较法.xlsx」），每个要用的类别各导一次；改了系数重导一次。")
    note(doc, "两条路都是「缺则补、已有不动」，可重复点/重复导，不会堆重复版本。")
    ...
    h2(doc, "基础表版本")
    p(doc, "Excel 是权威、系统存副本、改了重导一次。版本号是内容指纹（内容一变即变，改不了也伪造不了）。"
           "旧版本永不覆盖——日后复查旧报告时，能拿回当时那版基础表把结果重算出来。")
    h2(doc, "台账")
    ...
    h1(doc, "八、数据与措辞")
    h2(doc, "数据放在哪")
    p(doc, "单机模式下，数据以纯文本 JSON 存在程序旁的 data 文件夹里（实例库、草稿、各版基础表、生成台账），"
           "可直接打开看、手改、备份。联钉钉模式下，问卷、实例、台账汇总在公司钉钉多维表，公司统一留存。")
    h2(doc, "改措辞")
    table(doc, ["改什么", "改哪里"], [
